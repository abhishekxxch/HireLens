  ...
  bulletRewrites: z
    .array(
      z.object({
        ...
        rewritten: z.string().trim().max(500),
...
function applyRewritesToText(rawText, rewrites) {
  ...
  for (const r of rewrites) {
    ...
    if (idx >= 0) {
      ...
      // Fallback: append a strengthened alternative line
      result += `\n${r.rewritten}`;
    }
  }
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const sourceRewrites =
      req.body.bulletRewrites?.length
        ? req.body.bulletRewrites
        : analysis.bulletRewrites;

    const selected = req.body.rewriteIds?.length
      ? sourceRewrites.filter((r) =>
          req.body.rewriteIds.includes(r._id?.toString?.() || String(r._id))
        )
      : sourceRewrites;
    ...
    // Safety net: pre-build a structured copy from the
    // chosen bullets swapped in, so V2 never builds with empty sections even if
